...
public class JournalDaoImpl implements JournalDao {
    ...
    public List<Journal> getAllRequests() {
        Session session = sessionFactory.getCurrentSession();
        return session.createQuery("select j from Journal j join fetch j.bookID b join fetch j.userID u order by j.id").getResultList();
    }
    ...
    public List<Journal> usersWhoDidNoReturnBookOnTime() {
        Session session = sessionFactory.getCurrentSession();
        try {
            return session.createQuery("select j from Journal j join fetch j.bookID b join fetch j.userID u where ((j.bookReturnDate is null) and (j.expectedReturnDate <= current_date))")
                    .getResultList();
        } catch (NoResultException ex) {
            return null;
        }
